"""HTTP mode configuration for the futuresearch MCP server."""
...
from futuresearch_mcp.app import http_lifespan, no_auth_http_lifespan
from futuresearch_mcp.auth import FuturesearchAuthProvider, SupabaseTokenVerifier
from futuresearch_mcp.config import settings
from futuresearch_mcp.middleware import (
...
def configure_http_mode(
    *,
    ...
    auth_provider: FuturesearchAuthProvider | None = None
    if no_auth:
        ...
        auth_provider = FuturesearchAuthProvider(
            redis=redis_client,
...
def _register_routes(
    mcp: FastMCP,
    redis: Redis,
    auth_provider: FuturesearchAuthProvider | None,
) -> None:
    ...
    if auth_provider is not None:
        ...
        mcp.custom_route("/auth/start/{state}/{provider}", ["GET"])(
            auth_provider.handle_provider_start
        )
        mcp.custom_route("/auth/password", ["POST"])(
            auth_provider.handle_password_login
        )
