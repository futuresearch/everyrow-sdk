...
import futuresearch_mcp.tools  # noqa: F401
...
def _http_state(fake_redis):
    ...
    Also swaps futuresearch_results from stdio to HTTP variant, matching server.py startup.
    """
...
class TestMcpProtocol:
    ...
    async def test_call_progress_tool(self, _http_state):
        ...
        async with mcp_client() as session:
            with (
                ...
                patch("futuresearch_mcp.redis_store.PROGRESS_POLL_DELAY", 0),
            ):
    ...
    async def test_completed_progress_suggests_results(self, _http_state):
        """When progress reports completed, response tells to call futuresearch_results."""
        task_id = str(uuid4())
        status_resp = _mock_status_response(
            task_id=task_id,
            status="completed",
...
class TestMcpE2ERealApi:
    """Full pipeline through MCP protocol with real FutureSearch API."""

    @pytest.fixture
    def _real_client(self, _http_state):
        """Provide a real FutureSearch SDK client."""
