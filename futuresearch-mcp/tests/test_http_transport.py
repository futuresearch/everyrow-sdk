...
Flow tested:
  streamablehttp_client → HTTP /mcp → JSON-RPC → tool dispatch
    → FutureSearch API (real LLM) → poll → preview → paginate → CSV download

Prerequisites:
  - Redis on localhost:6379
  - FUTURESEARCH_API_KEY set (via env or ~/.claude/secrets/)
...
async def poll_via_mcp(
    ...
    """Poll futuresearch_progress via MCP tool call until terminal state.

    Returns the final human-readable text.
    """
    for i in range(max_polls):
        result = await session.call_tool(
            "futuresearch_progress", {"params": {"task_id": task_id}}
        )
        ...
        if "futuresearch_results" in human_text or "Completed:" in human_text:
            return human_text
