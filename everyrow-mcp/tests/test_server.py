...
class TestAgentUrlInput:
    """Tests for everyrow_agent with input_url — exercises model → tool → URL fetch → SDK."""

    @pytest.mark.asyncio
    async def test_submit_with_input_url(self):
        """input_url fetches CSV via httpx, passes DataFrame to SDK."""
        mock_task = _make_mock_task()
        mock_session = _make_mock_session()
        mock_client = _make_mock_client()
        ...
        with (
            ...
            params = AgentInput(
                task="Find HQ for each company",
                input_url="https://example.com/companies.csv",
            )
            result = await everyrow_agent(params)
    ...
    async def test_submit_with_google_sheets_url(self):
        ...
        with (
            ...
            params = AgentInput(
                task="Process sheet",
                input_url="https://docs.google.com/spreadsheets/d/ABC123/edit#gid=0",
            )
            await everyrow_agent(params)
    ...
    async def test_url_fetch_error_propagates(self):
        ...
        with (
            ...
            params = AgentInput(
                task="test",
                input_url="https://example.com/missing.csv",
            )
