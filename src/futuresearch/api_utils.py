...
def create_client() -> AuthenticatedClient:
    ...
    Reads FUTURESEARCH_API_KEY and FUTURESEARCH_API_URL from environment
    variables.
    ...
    """
    api_key = os.environ.get("FUTURESEARCH_API_KEY")
    if not api_key:
        raise ValueError("FUTURESEARCH_API_KEY is not set; cannot initialize client")
    api_url = os.environ.get("FUTURESEARCH_API_URL") or DEFAULT_FUTURESEARCH_API_URL
